...
export default function Analytics({ onNavigate }: Props) {
  ...
  if (error) return <Box><Text color={colors.error}>Error: {error}</Text></Box>;
  if (!snapshot) return <Box><Text>Loading...</Text></Box>;

  return (
    <Box flexDirection="column" padding={1}>
      <Text bold color={colors.primary}>Analytics Dashboard</Text>
      ...
      {snapshot.slaViolations.total > 0 && (
        <Box flexDirection="column" marginTop={1}>
          <Text bold color={colors.warning}>SLA Violations: {snapshot.slaViolations.total}</Text>
