...
describe("fuzzyMatch", () => {
  test("matches exact string", async () => {
    const { fuzzyMatch } = await import("../src/tui/ui/command-palette");
  ...
  test("matches scattered characters", async () => {
    ...
    expect(result.indices[2]).toBe(5);
  });
});

describe("COMMANDS list", () => {
  test("contains all expected views", async () => {
    const { COMMANDS } = await import("../src/tui/ui/command-palette");
    ...
    const expectedViews = [
      ...
      "streams", "delegate", "chat", "tdd", "theme", "help", "quit",
      "switch_account", "new_chat", "session_list",
      "send_message", "council_analysis", "health_check",
    ];
  ...
  test("command count is 29", async () => {
    const { COMMANDS } = await import("../src/tui/ui/command-palette");
